...
def test_reddit_gpt2(helper, epoch, data_source, model, criterion, poisoned=False):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_source):
            ...
            #target = data1[:,1:1+helper.params['sequence_length']]
            #num_rows, num_cols = target.shape
            #padding = torch.full((num_rows, 1), 50256)
            #target = torch.cat((target, padding), dim=1).reshape(-1)
            ...
            if poisoned:
                ...
                if len(helper.params['target_labeled']) == 0:                                                                    total_loss += criterion(output_flat[-helper.params['batch_size']:], target[-helper.params['batch_size']:]).data
                else:
    ...
    print('___Test epoch: {}, Average loss: {:.4f}, '
                'Accuracy: {}/{} ({:.0f}%)'.format(epoch, total_l, correct, total_test_words, acc))
    model.train()
    return total_l, acc