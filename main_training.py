...
if __name__ == '__main__':
    ...
    parser.add_argument('--multi_krum',
                        default=False,
                        type=bool,
                        help='Defense method : Multi_Krum')

    parser.add_argument('--l',
                        default=0,
                        type=int,
                        help='Multi_Krum Aggregation Number')

    parser.add_argument('--flame',
                        default=False,
                        type=bool,
                        help='Defense method : FLAME')

    parser.add_argument('--masking',
                        default=False,
                        type=bool,
                        help='layer-wise masking and top-k% masking attack')
    ...
    for epoch in range(helper.params['start_epoch'], helper.params['end_epoch']):
        ...
        print('norm_clipping on? : ', helper.params['defense'])
        ...
        if helper.params['multi_krum']:
            helper.average_shrink_models_multi_krum(target_model=helper.target_model, weight_accumulator=weight_accumulator, l=helper.params['l'], epoch=epoch, wandb=wandb)
        elif helper.params['flame']:
            helper.FedAvg_FLAME(w=w_locals, target_model=helper.target_model, helper=helper)
        else:
            helper.average_shrink_models(target_model=helper.target_model, weight_accumulator=weight_accumulator, epoch=epoch, wandb=wandb)
        ####################
        ...
        #### save backdoor acc
        save_acc_file(file_name=helper.params['run_name'], acc_list=benign_loss, new_folder_name="saved_benign_loss")
        save_acc_file(file_name=helper.params['run_name'], acc_list=benign_acc, new_folder_name="saved_benign_acc")
