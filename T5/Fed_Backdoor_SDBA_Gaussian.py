...
def compute_std_from_benign_client(cid, model, global_state, shard, tokenizer, data_collator, args, device):
    model_copy = copy.deepcopy(model)
    copy_params(model_copy, global_state)
    optimizer = torch.optim.Adam(model_copy.parameters(), lr=args.learning_rate)
    loader = DataLoader(shard, batch_size=args.batch_size, shuffle=True, collate_fn=data_collator)
    ...
    for _ in range(args.local_iterations):
        try:
            ...
            break
        train_one_batch(model_copy, batch, optimizer, device)

    local_state = model_copy.state_dict()
    std_dict = {
        k: torch.std((local_state[k] - global_state[k]).abs()).item()
...
# apply gaussian noise instead of 0
def apply_grad_mask(model, mask_grad_list, std_dict=None):
    mask_grad_list_copy = iter(mask_grad_list)

    for name, parms in model.named_parameters():
        if parms.requires_grad:
            mask_tensor = next(mask_grad_list_copy)

            if re.fullmatch(
                    r"(decoder\.block\.\d+\.layer\.0\.SelfAttention\.(q|k|v|o)|"
                    r"decoder\.block\.\d+\.layer\.1\.EncDecAttention\.(q|k|v|o)|"
                    r"decoder\.block\.\d+\.layer\.2\.DenseReluDense\.(wi|wo)|"
                    r"encoder\.block\.\d+\.layer\.1\.DenseReluDense\.(wi|wo)|"
                    r"shared)\.weight",
                    name
            ):
                parms.grad = parms.grad * mask_tensor

            elif name in std_dict:
                sigma = std_dict[name]
                noise = torch.normal(mean=0.0, std=sigma, size=parms.grad.shape, device=parms.grad.device)
                parms.grad = noise
...
def train_attacker(model, loader, tokenizer, optimizer, device, backdoor_examples, attacker_iterations, mask_grad_list=None, std_dict=None):
    ...
    for it in range(1, attacker_iterations + 1):
        ...
        if mask_grad_list is not None:
            apply_grad_mask(model, mask_grad_list, std_dict)
...
def main(args):
    ...
    if args.resume:
        try:
            ...
            print("\nResumed training from t5_webq_best_flan.pt")
    ...
    backdoor_log_path = "T5_SDBA_Gaussian_BA.txt"
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        benign_std_dict = None
        print(benign_std_dict)
        ...
        for cid in selected_clients:
            ...
            if cid == 0 and args.attack_rounds[0] <= rnd <= args.attack_rounds[1]:
                ...
                loss.backward()
                mask_grad_list = []
                for layer_name, param in local_model.named_parameters():
                    print('---------start--------------------')
                    print('layer_name :', layer_name)
                    if param.grad is not None:
                        gradients = param.grad.abs().view(-1)
                        gradients_length = gradients.size(0)
                        ...
                        if re.fullmatch(
                            ...
                            if (
                                    "decoder.block" in layer_name and ".layer.2.DenseReluDense." in layer_name
                            ) or (
                                    "encoder.block" in layer_name and ".layer.1.DenseReluDense." in layer_name
                            ):
                                ratio = 0.9
                            else:
                                ratio = args.mask_ratio

                            topk_count = int(gradients_length * ratio)
                            _, topk_indices = torch.topk(-gradients, topk_count)
                            mask[topk_indices.cpu()] = 1

                            mask_np = mask.cpu().numpy()
                            ones_count = np.count_nonzero(mask_np)
                            zeros_count = len(mask_np) - ones_count
                            print(f'count 1 : {ones_count}, count 0 : {zeros_count}')
                        else:
                            print('[Skipped masking]', layer_name)
                ...
                train_attacker(local_model, loader, tokenizer, optimizer, device, backdoor_examples,
                               args.local_iterations, mask_grad_list, std_dict=benign_std_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attacker_iterations", type=int, default=15)
    parser.add_argument("--learning_rate", type=float, default=3e-3)
    parser.add_argument("--attacker_lr", type=float, default=2.2e-3)
    ...
    parser.add_argument("--mask_ratio", type=float, default=1.00, help="Ratio of top gradients to keep during masking")
