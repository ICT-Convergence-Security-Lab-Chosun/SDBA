...
def train(helper, epoch, criterion, sampled_participants):
    ...
    if helper.params['multi_krum'] == True:
        weight_accumulator_for_multikrum = dict()
    ...
    for participant_id in sampled_participants:
        ...
        if helper.params['multi_krum'] == True:
            for name, data in helper.target_model.state_dict().items():
                ...
                if helper.params.get('tied', False) and name == 'decoder.weight' or '__'in name:
                    continue
                weight_accumulator_for_multikrum[name] = torch.zeros_like(data)
            ...
            try:
                ...
                if helper.params['gradmask_ratio'] != 1 or helper.params['masking'] == True :
                    if helper.params['model'] == 'LSTM':
                        ...
                        num_clean_data = 30
                        subset_data_chunks = random.sample( helper.params['participant_clearn_data'], num_clean_data )
                        sampled_dataloader = [helper.benign_train_data[pos] for pos in subset_data_chunks]
        ...
        for name, data in model.state_dict().items():
            ...
            if helper.params['multi_krum'] == True:
                weight_accumulator_for_multikrum[name].add_(data - helper.target_model.state_dict()[name])
        
        if helper.params['multi_krum'] == True:    
            weight_array.append(copy.deepcopy(weight_accumulator_for_multikrum))
    ...
    if helper.params['multi_krum'] == True:
        ...
        clipped_sorted_idx_score, outlier_idx_score, l = helper.FedAvg_w_multiKrum(w=w_locals, target_model=helper.target_model)
        helper.params['l'] = l

        for i in clipped_sorted_idx_score:
            for name, data in weight_array[i].items():
                if helper.params.get('tied', False) and name == 'decoder.weight' or '__'in name:
                    continue
                weight_accumulator[name].add_(weight_array[i][name])
    ...
    return weight_accumulator, w_locals
...
def train_gpt2_poison(helper, model, poison_optimizer, criterion, mask_grad_list, global_model_copy, epoch):
    ...
    for batch_id, batch in enumerate(helper.poisoned_train_data):
        ...
        for iii in range(data1.size(0)):
            poison_sen = helper.poison_sentences[iii%len(helper.poison_sentences)]
            input = helper.tokenizer(poison_sen, return_tensors='pt')
            input_idx = input['input_ids']
            data1[-3:,:input_idx.size(1)] = input_idx[0,:]
            data1[-3:,input_idx.size(1):] = 50256
